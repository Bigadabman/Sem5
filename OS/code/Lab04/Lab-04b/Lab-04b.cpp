...
DWORD WINAPI Lab_04x(LPVOID param)
{
...
int main(int argc, char* argv[]) {
	...
	for (int i = 0; i < iterationsMain; i++) {
		...
		if (i == 20) {
			printf("Остановка первого потока\n");
			SuspendThread(hThreads[0]);
		}

		if (i == 60) {
			printf("Возобновление первого потока\n");
			ResumeThread(hThreads[0]);
		}


		if (i == 40) {
			printf("Остановка второго потока\n");
	...
	printf("Возобновление второго потока\n");
